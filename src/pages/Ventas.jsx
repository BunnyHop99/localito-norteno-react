...
export default function Ventas() {
  ...
  const toggleRow = async (ventaId) => {
    ...
    if (newExpanded.has(ventaId)) {
      ...
      if (!venta.detalles || venta.detalles.length === 0) {
        ...
        newLoading.add(ventaId);
        setLoadingDetails(newLoading);
        
        try {
          console.log(`🔄 Cargando detalles de venta ${ventaId}...`);
          
          // Obtener la venta completa con detalles
          const ventaCompleta = await ventasService.getVenta(ventaId);
          
          console.log('📦 Venta completa recibida:', ventaCompleta);
          console.log('📦 Detalles:', ventaCompleta.detalles);
          
          // Actualizar la venta en el estado con los detalles
          setSales(sales.map(s => 
            s.id === ventaId 
              ? { ...s, detalles: ventaCompleta.detalles || ventaCompleta.items || [] }
              : s
          ));
        } catch (err) {
  ...
  const handleCompletarVenta = async () => {
    ...
    try {
      ...
      // Cerrar modal y limpiar PRIMERO para mejor UX
      setShowModal(false);
      limpiarFormulario();
      
      // Recargar datos para obtener la nueva venta con folio
      const [ventasData, productosData, estadisticasData] = await Promise.all([
        ventasService.getVentas(),
        inventarioService.getProductos(),
        ventasService.getEstadisticasHoy()
      ...
      console.log('📊 Estadísticas actualizadas:', estadisticasData);
      
      setSales(Array.isArray(ventasData) ? ventasData : ventasData.results || []);
      setProductos(Array.isArray(productosData) ? productosData : productosData.results || []);
      setEstadisticas(estadisticasData);
      
      // Obtener la venta más reciente (primera de la lista)
      const ventasArray = Array.isArray(ventasData) ? ventasData : ventasData.results || [];
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h3 className="text-2xl font-bold text-gray-900">
                ${(estadisticas?.monto_total || 0).toFixed(2)}
              </h3>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h3 className="text-2xl font-bold text-gray-900">
                ${estadisticas?.total_ventas > 0 
                  ? (estadisticas.monto_total / estadisticas.total_ventas).toFixed(2)
                  : '0.00'}
              </h3>
            </div>
          </div>
                              ...
                              <table className="w-full">
                                ...
                                <tbody className="divide-y divide-gray-200">
                                  {loadingDetails.has(sale.id) ? (
                                    ...
                                    const detalles = sale.detalles || sale.items || sale.lineas || sale.productos || [];
                                    
                                    console.log(`📦 Detalles de venta ${sale.folio}:`, detalles);
                                    ...
                                    return detalles.map((detalle, idx) => (
                                      <tr key={detalle.id || `${sale.id}-detalle-${idx}`} className="hover:bg-gray-50">
                                        <td className="px-4 py-2">
                                          <div>
                                            <div className="font-medium text-gray-900">
                                              {detalle.producto_nombre || detalle.nombre || detalle.producto?.nombre || 'Producto'}
                                            </div>
                                            {(detalle.producto_codigo || detalle.codigo || detalle.producto?.codigo) && (
                                              <div className="text-xs text-gray-500">
                                                Código: {detalle.producto_codigo || detalle.codigo || detalle.producto?.codigo}
                                              </div>
                                            )}
                                          </div>
                                        </td>
                                        ...
                                        <td className="px-4 py-2 text-right text-gray-600">
                                          ${parseFloat(detalle.precio_unitario || detalle.precio || 0).toFixed(2)}
                                        </td>
                                        <td className="px-4 py-2 text-right">
                                          <span className="font-semibold text-gray-900">
                                            ${(detalle.cantidad * parseFloat(detalle.precio_unitario || detalle.precio || 0)).toFixed(2)}
                                          </span>
                                        </td>
                                      </tr>
                                    ));
